...
export CC_PACKAGE_ID=$(peer lifecycle chaincode queryinstalled | sed -n -r "s/Package ID: (.*), Label: token.0$/\1/p")
echo $CC_PACKAGE_ID
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID mychannel --name token --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile ${FABRIC_HOME}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
